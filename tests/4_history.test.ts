...
test("show history on round that is not showed down", async ({ page, newPageOnNewContext: other, resetFirebase }) => {
  await resetFirebase();
  ...
  await expect(page.getByTestId("estimations/test@example.com/card")).not.toHaveText(/.+/);
  await expect(page.getByTestId("estimations/test2@example.com/card")).toBeVisible();
  await expect(page.getByTestId("estimations/test2@example.com/card")).not.toHaveText(/.+/);
});
