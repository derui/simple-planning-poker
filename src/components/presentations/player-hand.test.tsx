...
test("display player card as normal and not opened", async () => {
  render(<PlayerHand userName="user" displayValue="5" selected={false} userMode="normal" opened={false} />);
...
test("display player card as inspector", async () => {
  render(<PlayerHand userName="user" displayValue="5" selected={false} userMode="inspector" opened={false} />);
...
test("selected card", async () => {
  render(<PlayerHand userName="user" displayValue="5" selected={true} userMode="normal" opened={false} />);

  expect(screen.getByTestId("card").dataset).toHaveProperty("state", "handed");
});

test("opened card that user not selected", async () => {
  render(<PlayerHand userName="user" displayValue="5" selected={false} userMode="normal" opened={true} />);

  expect(screen.getByTestId("card").dataset).toHaveProperty("state", "notSelected");
});

test("opened card that user selected", async () => {
  render(<PlayerHand userName="user" displayValue="5" selected={true} userMode="normal" opened={true} />);
