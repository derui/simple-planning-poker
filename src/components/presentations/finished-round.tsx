import type { MouseEvent } from "react";
import classNames from "classnames";
import { BaseProps, generateTestId } from "../base";

export interface Props extends BaseProps {
  onClick?: (id: string) => void;
  id: string;
  theme?: string;
  finishedAt: Date;
  averagePoint: number;
}

const Styles = {
  root: classNames(
    "flex",
    "w-full",
    "rounded",
    "border",
    "border-secondary2-400",
    "px-3",
    "py-2",
    "items-center",
    "cursor-pointer",
    "transition-shadow",
    "hover:shadow-lg"
  ),
  container: classNames("flex-1", "flex", "flex-row", "ml-2", "items-center", "justify-center"),
  theme: (noTheme: boolean) =>
    classNames("flex-1", "ml-2", { "text-gray": noTheme, underline: !noTheme, "underline-offset-2": !noTheme }),
  date: classNames("flex-none", "text-sm", "flex", "flex-col", "justify-center"),
  dateTime: classNames("text-sm", "text-gray"),
  point: classNames("flex", "items-center", "rounded", "bg-secondary2-500", "text-white", "p-2"),
} as const;

const format = function format(dateTime: Date) {
  const year = `${dateTime.getFullYear()}`;
  const month = `${dateTime.getMonth() + 1}`;
  const date = `${dateTime.getDate()}`;
  const hour = `${dateTime.getHours()}`;
  const minute = `${dateTime.getMinutes()}`;
  const seconds = `${dateTime.getSeconds()}`;

  return `${year}/${month.padStart(2, "0")}/${date.padStart(2, "0")} ${hour.padStart(2, "0")}:${minute.padStart(
    2,
    "0"
  )}:${seconds.padStart(2, "0")}`;
};

// eslint-disable-next-line func-style
export function FinishedRound(props: Props) {
  const gen = generateTestId(props.testid);
  const theme = props.theme ?? "No theme";

  const handleClick = (e: MouseEvent) => {
    if (props.onClick) {
      e.stopPropagation();
      e.preventDefault();
      props.onClick(props.id);
    }
  };

  return (
    <li className={Styles.root} onClick={handleClick} data-testid={gen("root")}>
      <span className={Styles.point} data-testid={gen("point")}>
        {props.averagePoint.toString()}
      </span>
      <span className={Styles.container}>
        <span className={Styles.theme(!props.theme)} data-testid={gen("theme")}>
          {theme}
        </span>
        <span className={Styles.date} data-testid={gen("date")}>
          Finished at <span className={Styles.dateTime}>{format(props.finishedAt)}</span>
        </span>
      </span>
    </li>
  );
}
